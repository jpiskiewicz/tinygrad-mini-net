...
class ExpandSqueeze:
  def __init__(self, in_channels: int, kernel_size: int):
    self.depthwise = Conv2d(in_channels, in_channels, kernel_size, groups=in_channels)
...
class DualMultiscaleResidual:
  ...
  def __call__(self, x: Tensor) -> Tensor:
    x1 = self.expandSqueeze1(x).add(self.expandSqueeze2(x))
    x2 = self.bn(self.conv(x1.relu())).add(x1)
    return self.expandSqueeze1(x2).add(self.expandSqueeze2(x2)).add(self.bn(self.conv(x))).relu()
...
class EncoderBlock:
  def __init__(self, in_channels: int):
    ...
    self.strided_conv = Conv2d(in_channels, out_channels, 3, 2)
...
class DecoderBlock:
  def __init__(self, in_channels: int):
    out_channels = in_channels // 2
    self.transpose_conv = ConvTranspose2d(in_channels, out_channels, 3)
